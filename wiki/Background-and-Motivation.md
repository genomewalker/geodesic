...
The sphere approach avoids all-vs-all pairwise comparisons. Each genome is sketched once and projected to a unit vector via [Nyström spectral embedding](https://en.wikipedia.org/wiki/Nystr%C3%B6m_method): the anchor Gram matrix is built from Jaccard similarities among a small anchor set ($n_\text{anchors} \ll n$), eigendecomposed once, and each remaining genome is projected into the same spectral basis at $O(n_\text{anchors})$ cost. Total comparison cost is $O(n_\text{anchors}^2 + n \cdot n_\text{anchors})$, linear in $n$ for fixed anchor count, replacing the $O(n^2)$ all-pairs baseline. Representative selection then operates on precomputed embedding vectors via [HNSW](https://doi.org/10.1109/TPAMI.2018.2889473) (Malkov & Yashunin 2018), with OPH-sketch verification for borderline pairs near the ANI threshold (Phase 7) and a universal post-selection certification pass (Phase 8) that promotes any uncovered genome to a representative.

OPH signatures encode k-mer set structure in a form amenable to Nyström spectral embedding: dual-sketch averaged Jaccard gives the anchor Gram matrix, and the Mash distance formula relates Jaccard to ANI — from raw sequence to a diversity score with no training data.
