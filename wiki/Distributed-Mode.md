...
| `--tmp-dir` | `<scatter-dir>` | Temporary directory for workers (defaults to the scatter output dir itself — deliberately not `/tmp`, so temp files never land there by accident) |
