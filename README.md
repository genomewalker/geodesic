...
2. **Embed** — project all genomes onto a unit sphere via a [Nyström-style](https://en.wikipedia.org/wiki/Nystr%C3%B6m_method) embedding heuristic with ridge regularisation (not a formal Mercer kernel approximation: the blended kernel is not guaranteed PSD, and Laplacian normalisation changes the inner-product semantics), with adaptations for incomplete assemblies and uneven sampling:
