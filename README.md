...
2. **Embed** — project all genomes onto a unit sphere via a [Nyström-style](https://en.wikipedia.org/wiki/Nystr%C3%B6m_method) embedding heuristic (not a formal Mercer kernel approximation: Laplacian normalisation changes inner-product semantics; dual-sketch asymmetry breaks Nyström coherence), with adaptations for incomplete assemblies and uneven sampling:
   ...
   - *Averaged kernel*: anchor Gram matrix uses dual-sketch average $K[i,j] = (J_1+J_2)/2$, halving Jaccard estimation variance. Non-anchor extension uses sig1 only (engineering trade-off; see dual-sketch asymmetry note in ALGORITHM.md).
   - *Pure Jaccard kernel*: anchor matrix uses Jaccard only; containment blend was removed to ensure PSD-compatible input to the eigendecomposition.
   - *PSD ridge repair*: if the anchor matrix is indefinite after eigendecomposition ($\lambda_\text{min} < -10^{-8}$), the eigenvalue spectrum is shifted in-place by $|\lambda_\text{min}| + 10^{-6}$ (no re-decomposition; eigenvectors are invariant under diagonal shifts).
   - *Regularisation*: symmetric Laplacian normalisation ($D^{-1/2} K D^{-1/2}$) removes hub-anchor bias; Tikhonov regularisation ($\lambda = 0.01\,\bar\mu_K$, floor $10^{-4}$) prevents eigenvalue blow-up.
